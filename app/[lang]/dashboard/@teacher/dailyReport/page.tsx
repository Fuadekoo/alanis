...
import {
  createTeacherReport,
  deleteTeacherReport,
  getTeacherStudentsCalendar,
} from "@/actions/teacher/teacherReport";
...
export default function Page() {
  ...
  // Get all teacher's students with calendar data
  ...
  const handleDeleteConfirm = async () => {
    ...
    setIsDeleting(true);
    try {
      const result = await deleteTeacherReport(reportToDelete.id);
      if (result.success) {
        showAlert({
          message: isAm
            ? "ሪፖርት በተሳካ ሁኔታ ተሰርዟል!"
  ...
  const handleCreateReport = async () => {
    if (!selectedStudent || !modalDate || !learningProgress) {
      showAlert({
        message: isAm
          ? "እባክዎ ሁሉንም መስኮች ይሙሉ"
    ...
    setIsCreating(true);
    try {
      const result = await createTeacherReport({
  ...
  const handleOpenStatusModal = (
    ...
    // Get student's time slot from calendar data
            ...
            <div className="flex flex-col gap-3">
              <div className="overflow-auto">
                <table className="w-full border-collapse text-sm">
                  <thead className="sticky top-0 bg-default-100 dark:bg-default-900/80 backdrop-blur z-30">
                    <tr>
                      <th className="border border-default-200/70 p-2 text-left font-semibold min-w-[180px] sticky left-0 bg-default-100 dark:bg-default-900/80 z-30">
                        {isAm ? "ተማሪ" : "Student"}
                      </th>
                      <th className="border border-default-200/70 p-2 text-left font-semibold min-w-[120px] bg-default-100 dark:bg-default-900/80">
                      ...
                      paginatedCalendarData.map(
                        (item: CalendarRow, rowIndex: number) => {
                          ...
                          return (
                            ...
                            >
                              <td
                                className={`border border-default-200/70 p-2 font-medium sticky left-0 z-20 text-default-700 dark:text-default-200 ${stickyBgClass}`}
                              >
                              ...
                              <td
                                className={`border border-default-200/70 p-2 text-default-600 dark:text-default-300 ${rowBgClass}`}
                              >
                                {item.timeSlot ? (
                                  <Chip
                                    size="sm"
