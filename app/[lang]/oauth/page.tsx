...
export default function OAuthLoginPage() {
  ...
  const handleOAuthCallback = useCallback(async () => {
    ...
    try {
      ...
      if (!codeResponse.ok) {
        ...
        throw new Error(
          errorData.error_description || "Failed to generate code",
        );
      }
      ...
      window.open(callback.toString(), "_blank");
    } catch (error) {
      console.error("OAuth callback error:", error);
      setError(
        error instanceof Error
          ? error.message
          : "Failed to process OAuth. Please try again.",
      );
  ...
  const { onSubmit, validationErrors, register, isLoading } = useRegistration(
    authenticate,
    loginSchema,
    async (state) => {
      if (state.status) {
          ...
          <p className="text-sm text-gray-600">
            {isRedirecting
              ? lang == "am"
                ? "ወደ ሌላ ስርዓት በመመለስ ላይ..."
                : lang == "or"
                  ? "Sagantaa biraa deebi'aa..."
                  : "Redirecting to external system..."
              : lang == "am"
                ? "በመጫን ላይ..."
                : lang == "or"
                  ? "Hordofaa..."
